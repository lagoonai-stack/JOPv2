...
# Agent definitions for OpenAI. Each agent has a system prompt and optional model override.
# Use ENV["OPENAI_API_KEY"] for the API key.
Rails.application.config.openai_agents = {
  "movie-maker" => {
    system_prompt: <<~PROMPT.strip,
      ...
      ## PROIBIDO nas respostas conversacionais (Passos 1 a 5):
      ...
      EXCEÇÃO ÚNICA — Passo 6 (prompt final): o bloco entregue ao#{' '}
      usuário para colar no Antigravity/Cursor DEVE usar markdown#{' '}
      completo (headers, tabelas, code blocks). É um documento técnico,#{' '}
      não uma mensagem de conversa. Fora do Passo 6, a proibição#{' '}
      acima é absoluta.
      # PERMITIDO nas respostas conversacionais (únicos formatos aceitos):
      ...
      - Para o prompt final (Passo 6): use markdown completo conforme descrito acima
      ## REGRA DE OURO:
      ...
      Antigravity/Cursor tem um LIMITE MÁXIMO de 25.000 tokens.
      ...
      NUNCA ultrapassar 25.000 tokens no prompt final.
