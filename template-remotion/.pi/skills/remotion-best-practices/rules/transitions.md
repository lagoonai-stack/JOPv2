...
> [!WARNING]
> **ERRO FATAL DE COMPILAÇÃO:** Um `<TransitionSeries.Transition />` **NUNCA** pode ser seguido ou precedido por um `<TransitionSeries.Overlay />`.
>
> A estrutura OBRIGATÓRIA é sempre intercalar com um `Sequence`:
> `Sequence` -> `Transition` -> `Sequence` -> `Overlay` -> `Sequence`
>
> Se você colocar um Transition e um Overlay juntos (ex: `Transition` -> `Overlay`), **o Remotion vai quebrar com um Runtime TypeError**.

Transitions and overlays can coexist in the same `<TransitionSeries>`, but they must ALWAYS be separated by a `<TransitionSeries.Sequence>`.
